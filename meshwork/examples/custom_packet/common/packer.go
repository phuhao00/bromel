...
func (p *CustomPacker) Pack(entry *meshwork.Entry) ([]byte, error) {
	// format: totalSize(4)|idSize(2)|id(n)|data(n)

	id, err := meshwork.ToStringE(entry.ID)
	if err != nil {
		return nil, err
	}
...
func (p *CustomPacker) Unpack(reader io.Reader) (*meshwork.Entry, error) {
	// format: totalSize(4)|idSize(2)|id(n)|data(n)
